...
get.data <- function(x){
    ...
    to.parse$Name <- as.factor(to.parse$Name) # cleanup - convert coin Name to factor
    to.parse <- to.parse[ -c(1) ] # remove first column (duplicate)
    to.parse$Name <- factor(to.parse$Name,
                              levels = to.parse$Name[order(to.parse$'24h')])  # sort by gain value
...
server <- function(input, output) {
    ...
    # P L O T   O U T P U T
    output$plot <- renderPlot({ (ggplot(data=liveish_data(), aes(x=Name, y=`24h`)) +
